...
> @datetime: 2025-12-22 11:43:13 UTC+08:00
...
- 2025-12-22：完成项目的初步规划与环境搭建, 确定使用Scrapy框架进行数据爬取; 分析数据来源, 制定爬虫策略; 设计数据库表结构.
- 2025-12-23：实现获取电影数据的ID, 使用 Redis 缓存来进行管理整个电影数据 Task 的数据获取进度. 完善爬虫代码, 新建 Pipeline 数据持久化; 解析电影信息数据,获取电影名称、导演、主演、类型、上映时间等基本信息.
- 2025-12-24：完善电影信息的泳道图, 拆分获取电影信息和电影短评的爬虫任务; 实现电影信息入库和电影演员入库, 电影ID和演员ID的关联关系入库.
- 2025-12-25：实现电影信息全部自动入库, 优化缓存机制, 数据库改为 PostgreSQL;, 重构数据获取翻页逻辑, 增加电影类型筛选, Spider 增加代理请求.
- 2025-12-26：优化电影信息获取, 完善电影 ID 获取逻辑, 获取数据改为单线程应对豆瓣的反爬机制, 增加入库电影 ID 和已获取电影 ID 的对比逻辑, 避免重复获取数据, 解析电影短评数据
